...
# server function, getting values from 'input' and assign values to 'output'
function(input, output) {
  # Read the Data File to begin with
  dataframe<-read.csv("housing.csv")
  ...
  # Fill up Test file with an additional field of the predicted values
  test$SalePrice<-rep(NA,1459)
  # Merge Training and test data
  house<-bind_rows(train,test)
  # Get all the Categorical values out if the data set
  cat_var <- names(train)[which(sapply(train, is.character))]
  # Get Aditional fields in the Categorical character values
  cat_car <- c(cat_var, 'BedroomAbvGr', 'HalfBath', ' KitchenAbvGr','BsmtFullBath', 'BsmtHalfBath', 'MSSubClass')
  # Get all the numerical values of the data set
  numeric_var <- names(train)[which(sapply(train, is.numeric))]
  # Get a List of categorical values, This is done as we can then show them as dropdown
  train1_cat<-train[cat_var]
  # Get a list of Label values from the colector
  train1_num<-train[numeric_var]
  # Remove any Null Value in all numerical values of the data
  correlations <- cor(na.omit(train1_num[,-1]))
  # Get Row indices out of the corelated values 
  row_indic <- apply(correlations, 1, function(x) sum(x > 0.3 | x < -0.3) > 1)
  # Get the Correlation values indices
  correlations<- correlations[row_indic ,row_indic ]
  # Pick up the outliers out of the trainig  data set
  train <- train[train$GrLivArea<=4000,]
  # Check if there is a row that has any missing values in the Sum
  ...
  # Remove Id value of the Dataset
  ...
  house_model <- randomForest(SalePrice~.,
                              data = train_new)
  ## 9...Final Prediction
  # Depending upon the data that has been selected, Predict using the test set
  ...
  # analogies of 'get' functions, if you need the value from the input, call the appropriate function
  # if it satisfies conditions, you get your value
  
  # return neighborhood names which are consistent with data set, based on users input
  neighborhoodInput <- reactive({
    switch(input$neighborhood,
           "Any neighborhood",
  ...
  # returns house quality value
  HouseQualityInput <- reactive({
    input$House_quality
  })
  
  # returns square footage value
  AreaInput <- reactive({
    if(is.integer(input$living_area) & input$living_area > 0 & input$living_area < 1000000000){
      input$living_area  
    } else {
      NULL
    }
  ...
  # creatung vector with exterier quality consistent with data set
  ExterQualVector <- c("Po","Fa","TA","Gd","Ex");
  
  # return value based on index\input from 1 to 5 
  ExteriorInpit <- reactive({
    ExterQualVector[input$ExterQual]
  })
  
  # return year value if it pass validation
  YearBuiltInput <- reactive({
    if(input$year_built > 1879 & input$year_built <= year(Sys.Date()) & is.integer(input$year_built)){
      input$year_built  
    } else {
      NULL
    }
  ...
  # this function render some text information about user's inputs and assighns it to output element
  output$choice <- renderPrint({
    
    neighborhood <- input$neighborhood
    quality <- HouseQualityInput()
    square <- AreaInput()
    exterior_quality <- input$ExterQual
    year <- YearBuiltInput()
    
    cat("Choosen neighborhood:",neighborhood,"\n",
        "Choosen quality:",quality,"\n",
        "Living area in sq. feet:", square,"\n",
        "Exterior quality:", exterior_quality,"\n",
        "Original construction date:", year
    )
  })
  
  # this function render text information:
  # price range
  # and user's inputs, whih our prediction is based on
  # and assighns it to the output element
  output$prediction <- renderPrint({
    
    # assighn "Incorrect inputs" value to the range price
    # it will not be changed if inputs don't pass validation
    rows <- "Incorrect inputs"
    ...
    year <- YearBuiltInput()
      ...
      if(!is.null(AreaInput()) & !is.null(YearBuiltInput())){
        test_print <- filter(test,
                             test$GrLivArea <= AreaInput()  & test$OverallQual <= HouseQualityInput()
                & test$YearBuilt <= YearBuiltInput()) %>%
          arrange(Id)
      ...
      cat("Predicted price range in thousands of dollars:", rows,"\n",
          "Choosen quality:",quality,"\n",
          "Living area in sq. feet:", area,"\n",
          "Original construction date:", year)
  })
  

  # next 2 functions filter dataset differently based on different user's inputs
  # this one based only on neighborhood and area of the house
  dataset_neighb_area <- reactive({
    if(!is.null(AreaInput()) & neighborhoodInput() != "Any neighborhood"){
      filter(dataframe, dataframe$GrLivArea <= AreaInput() & dataframe$Neighborhood == neighborhoodInput())
    } else if(!is.null(AreaInput()) & neighborhoodInput() == "Any neighborhood"){
      filter(dataframe, dataframe$GrLivArea <= AreaInput())
    }
  })
  
  # this one based on all inputs
  dataset_all_inputs <- reactive({
    if(!is.null(AreaInput()) & !is.null(YearBuiltInput()) & neighborhoodInput() != "Any neighborhood"){
      filter(dataframe,
             dataframe$GrLivArea <= AreaInput() & dataframe$Neighborhood == neighborhoodInput() & dataframe$OverallQual <= HouseQualityInput()
             & dataframe$ExterQual %in% ExterQualVector[1:input$ExterQual] & dataframe$YearBuilt <= YearBuiltInput())
    } else if(!is.null(AreaInput()) & !is.null(YearBuiltInput()) & neighborhoodInput() == "Any neighborhood"){
      filter(dataframe,
             dataframe$GrLivArea <= AreaInput() & dataframe$OverallQual <= HouseQualityInput()
             & dataframe$ExterQual %in% ExterQualVector[1:input$ExterQual] & dataframe$YearBuilt <= YearBuiltInput())      
    }
  })
    
  
  # this function renders plot based on the data which is filtered by neighborhood and area of the house
  # and assigns it to the output element
  output$plot <- renderPlot({
      if(!is.null(AreaInput())){
        p <- ggplot(as.data.frame(dataset_neighb_area()), aes(y=SalePrice,x=GrLivArea)) 
        p <- p + geom_point() + labs(title = "Sale prices based on neighborhood and housing area",
                                     x = "Area", y = "Price")
      }
      print(p)
  ...
  # this function renders plot based on all inputs
  # and assigns it to the output element
  output$plot2 <- renderPlot({
    if(!is.null(AreaInput()) & !is.null(YearBuiltInput())){
      p <- ggplot(as.data.frame(dataset_all_inputs()), aes(y=SalePrice,x=GrLivArea))
      p <- p + geom_point() + labs(title = "Sale prices based on all parameters",
                                   subtitle = "Results are based on input and all lesser values",
                                   x = "Area", y = "Price")
    }
    print(p)
  ...
  # Validation
  # area validation function, which sets conditions and defins message
  
  area_validation <- reactive({
    validate(
      need(is.integer(input$living_area) & input$living_area > 0 & input$living_area < 1000000000, "Please input a valid number")
    )
  })
  
  # year validation function, which sets conditions and defins message
  year_validation <- reactive({
    validate(
      need(input$year_built > 1879 & input$year_built <= year(Sys.Date()) & is.integer(input$year_built), 
           "Please input a valid year")
    )
  })
  
    
    # Set values to validation messages and show them if validation failed
    output$validation_error_area <- renderPrint({ area_validation() })
    output$validation_error_year <- renderPrint({ year_validation() })
